...
public class EditorActivity extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Check intent that called this activity and get its data
        Intent intent = getIntent();
        mCurrentPetUri = intent.getData();

        // Set activity title depending on the data extracted from the intent
        ...
        // Set up onTouchListeners on all input fields to determine whether the user has modified
        // them in order to prompt a dialog for quitting without saving
    ...
    private void savePet() {
        ...
        // If weight is not provided use 0 by default
        int weight = 0;
    ...
    private void deletePet() {
        // Only perform the delete if there is an existing pet
        if (mCurrentPetUri != null){
            int rowsDeleted = getContentResolver().delete(mCurrentPetUri, null, null);

            // Show toast message depending on delete outcome
            if (rowsDeleted > 0) {
        ...
        // Pet was deleted, close activity
        finish();
    }
}